...
BranchAndBound::BranchAndBound(ClusteringProblem const &input)
  ...
  _columnGenerator.setExact(_input->getExactOracle(), _master->minus_dual(),
                            _decision);
  _columnGenerator.setVns(_input->getVnsOracle(), _master->minus_dual(),
                          _decision);
...
//			_master->udpate_stabilization();
    if (_output != NULL) {
      ...
      output() << std::setw(25) << std::scientific << std::setprecision(10)
               << _master->obj();
      output() << std::setw(25) << std::scientific << std::setprecision(10)
               << rc;
      output() << std::setw(25) << std::scientific << std::setprecision(10)
