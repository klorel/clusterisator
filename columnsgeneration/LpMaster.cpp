...
void LpMaster::add(Column const & column, ColumnBuffer & columnBuffer,
  ...
  if (!result.second) {
    ...
    columnBuffer.add(column.cost(), _solver.continuous(), 0, _solver.infinity(),
...
void LpMaster::addSingleton() {
  for (int v(0); v < _input->nV(); ++v) {
    ...
    column.reducedCost() = column.computeReducedCost(minus_dual());
    add(column);
  }
}
...
bool PrimalPredicate::operator()(DoubleVector const & v1,
                                 DoubleVector const & v2) const {
  //return v1 < v2;
...
bool PrimalPredicate::operator()(Int2Double const & v1,
                                 Int2Double const & v2) const {
