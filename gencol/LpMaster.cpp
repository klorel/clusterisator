...
LpMaster::LpMaster(BipartiteGraph const &input) :
		_input(input), _env(NULL), _lp(NULL) {
	int err;
	_env = CPXopenCPLEX(&err);
	CPXsetintparam(_env, CPX_PARAM_SCRIND, CPX_OFF);
	initLp();
	_rAndbInColumn.resize(_input.nR(),
			std::vector<std::list<Column const *> >(input.nB()));
...
void LpMaster::initLp() {
	...
	CPXaddrows(_env, _lp, (int) _input.nV(), (int) _input.nV(),
			(int) _input.nV(), ones.data(), equal.data(), rmatbeg.data(),
			rmatind.data(), rmatval.data(), NULL, NULL);
	CPXchgobjsen(_env, _lp, -1);
	_index.clear();
	_index.reserve(_primal.size());
	size_t i(0);
	// donn�es li�es au singleton
...
void LpMaster::add(Column const & column) {
	...
	if (!result.second) {
		std::cout << " column.reducedCost() is " << std::setprecision(20)
				<< column.reducedCost() << std::endl;
		writeColumns("bug.txt");
	...
	for (auto const & r : column.first) {
		cmatind.push_back((int) r);
	}
	for (auto const & b : column.second) {
		cmatind.push_back((int) (_input.nR() + b));
	}

	CPXaddcols(_env, _lp, ccnt, nzcnt, obj.data(), cmatbeg.data(),
			cmatind.data(), cmatval.data(), lb.data(), ub.data(), NULL);
...
//	CPXgetobjval(_env, _lp, _obj.data());
}
...
void LpMaster::applyBranchingRule(Node const & node, std::vector<double> & obj) {
	// on p�nalise les colonnes violant les r�gles
	std::list<Column const *> const & columns(
			node.cannot() ?
					_rAndbInColumn[node.r()][node.b()] :
					_rOrbInColumn[node.r()][node.b()]);
