...
soit $A = (V_1, ... V_n)$, on peut alors écrire :
...
À partir de là, notons $\lambda$ une valeur propre de $K$ et $X$ un vecteur propre associé.
On a donc : 
...
D'où l'on déduit : $\lambda \in \mathbb R^+$.
\\[2.5em]
...
\end{center}
Notant $w = \sum_{1\leq i\leq k}{a_iV_i}$, on trouve que $w$ est orthogonal à tous les $V_i$, donc à lui-même.
...
\\[2.5em]
Donc $K$ est strictement définie positive si et seulement si les $V_i$ sont linéairement indépendants.
Autrement, elle est seulement semi-définie positive.
