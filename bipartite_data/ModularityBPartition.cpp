...
ModularityBPartition::ModularityBPartition(BipartiteGraph const & input, int k)
    : Partition(input.nV(), k),
      _input(&input) {
  DoubleVector w(input.nV());
  for (int r(0); r < input.nR(); ++r)
    w[r] = input.k(r);
  for (int b(0); b < input.nB(); ++b)
...
Double ModularityBPartition::score(int k) const {
  Double result(0);
...
Double ModularityBPartition::scoreIfSwap(int k, int node) const {
  Double result(0);
  for (Edge const & edge : _input->edges()) {
