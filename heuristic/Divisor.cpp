...
Double Divisor::buildLp(ModularityBPartition const & partition, int label) {
  Double Rc(0);
...
//	MY_PRINT(cst_obj);
  ...
  ColumnBuffer columns(CPX_CONTINUOUS);
  // 0..R-1 : Yr
  // R..R+B-1 : Yb
  _y.clear();
  ...
  int const tR((int) std::ceil((std::log(Rc + 1) / log(2) - 1)));
  int const tB((int) std::ceil((std::log(Bc + 1) / log(2) - 1)));
  IntVector a(tR + 1);
  ...
  for (int h(0); h < tR + 1; ++h) {
    for (int l(0); l < tB + 1; ++l) {
      ...
      columns.add(-std::pow(2, 1 + l + h) * _input->inv_m() * _input->inv_m(),
      CPX_CONTINUOUS,
                  0, CPX_INFBOUND, GetStr("a_", h, "_", l));

    }
  }
  int const R1(columns.size());
  columns.add(+Bc * _input->inv_m() * _input->inv_m(), CPX_CONTINUOUS,
              -CPX_INFBOUND, +CPX_INFBOUND, "R1");

  int const B1(columns.size());
...
// Srb <= Yr (r,b) in E
  for (Edge const & edge : _input->edges()) {
    if (partition.label(_input->nR() + edge._j) == label
        && partition.label(edge._i) == label) {
      // Srb <= Yr (r,b) in E
      rows.add(0, 'L', GetStr("S_R", edge._i, "_B", edge._j, "_YR_", edge._i));
      rows.add(_y[edge._i], -1);
      rows.add((int) s.get(edge._i, edge._j), 1);
      // Srb <= Yb (r,b) in E
      rows.add(0, 'L', GetStr("S_R", edge._i, "_B", edge._j, "_YB_", edge._j));
...
bool Divisor::run(ModularityBPartition & p, int label) {
//MY_PRINT(label);
...
//	success = true;
  if (success) {
    ...
    int const new_label((int) p.getUnUsedLabel());
