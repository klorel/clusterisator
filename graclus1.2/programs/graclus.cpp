...
int main(int argc, char *argv[]) {
	...
	printf("  Name: %s, \n  #Vertices: %d, #Edges: %d, ", filename, graph.nvtxs,
			graph.nedges / 2);
	if (graph.ncon > 1)
	...
	if (clusteringEva > 0) {
		...
		if (graph.ncon == 1) {
			...
			MLKKM_PartGraphKway(&graph.nvtxs, graph.xadj, graph.adjncy,
					graph.vwgt, graph.adjwgt, &wgtflag, &numflag, &nparts,
	...
	GKfree((void **) &graph.xadj, (void **) &graph.adjncy,
			(void **) &graph.vwgt, (void **) &graph.adjwgt, (void **) &part,
			LTERM);
}
