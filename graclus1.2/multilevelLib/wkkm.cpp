...
void transform_matrix_half(CtrlType *ctrl, GraphType *graph, idxtype *w,
		float *m_adjwgt)
...
// do batch-local search; chain_length is the search length
{
	...
	do {
		...
		if (chain_length > 0) {
			...
			//					tpwgts, ubfactor);
			moves = local_search(ctrl, graph, nparts, chain_length, w, tpwgts,
					ubfactor);
...
void Weighted_kernel_k_means(CtrlType *ctrl, GraphType *graph, int nparts,
	...
	*bndptr, *bndind;
	...
	squared_inv_sum = fmalloc(nparts,
			"Weighted_kernel_k_means: squared sum inverse");
	squared_sum = idxsmalloc(nparts, 0,
...
//return # of points moved
{
	...
	*bndind;
	...
	accum_change = fmalloc(chain_length + 1,
	...
	for (k = 0; k < nparts; k++)
		if (sum[k] > 0)
			...
			for (ii = 0; ii < loopend; ii++)
				...
				kDist[ii][k] = squared_sum[k] / (1.0 * sum[k] * sum[k])
				- 2 * kDist[ii][k] / sum[k];
				...
				if (sum[me] > w[j]) // if this cluster where j belongs is not a singleton
					for (k = 0; k < nparts; k++)
						if (k != me) {
							...
							tempchange = -kDist[ii][me] * sum[me] * w[j]
							                                          / (sum[me] - w[j])
							                                          + kDist[ii][k] * sum[k] * w[j]
							                                                                      / (sum[k] + w[j]);
...
float onePoint_move(GraphType *graph, int nparts, idxtype *sum,
		idxtype *squared_sum, idxtype *w, idxtype *self_sim, int **linearTerm,
		int ii) {

	int k, j, s, i, nbnd, temp, q1, q2, minchange_id, new_squared_sum1,
	new_squared_sum2, me, nedges, nvtxs, loopend;
...
void move1Point2EmptyCluster(GraphType *graph, int nparts, idxtype *sum,
		idxtype *squared_sum, idxtype *w, idxtype *self_sim, int **linearTerm,
		int k) {
	int j, s, ii, nbnd, q1, q2, minchange_id, new_squared_sum1,
	new_squared_sum2, me, nvtxs, loopend;
...
void remove_empty_clusters_l2(CtrlType *ctrl, GraphType *graph, int nparts,
	...
	if (number_of_empty_cluster > 0) {
		...
		*bndind, *self_sim, nbnd;
		...
		for (k = 0; k < nparts; k++)
			if (clustersize[k] == 0) {
				move1Point2EmptyCluster(graph, nparts, sum, squared_sum, w,
						self_sim, linearTerm, k);
			}
		free(sum);
		free(squared_sum);
		free(self_sim);
...
void MLKKMRefine(CtrlType *ctrl, GraphType *orggraph, GraphType *graph,
	...
	/* Compute the parameters of the coarsest graph */ComputeKWayPartitionParams(
			ctrl, graph, nparts);
	...
	for (i = 0;; i++) {
		...
		if (graph->vwgt == NULL) {
			graph->vwgt = idxsmalloc(graph->nvtxs, 1,
					"RefineKWay: graph->vwgt");
			graph->adjwgt = idxsmalloc(graph->nedges, 1,
					"RefineKWay: graph->adjwgt");
			mustfree = 1;
		}
