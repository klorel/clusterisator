...
void XpressSolver::add(RowBuffer & buffer) {
	...
	nReturn = XPRSaddrows(_lp, buffer.size(), buffer.nz(), buffer.type(), buffer.rhsObj(), NULL,
		buffer.begin(), buffer.index(), buffer.value());
	if (nReturn)errormsg("XPRSaddrows", __LINE__, nReturn);
...
void XpressSolver::add(ColumnBuffer & buffer) {
	...
	if (nReturn)errormsg("XPRSaddcols", __LINE__, nReturn);
	if (!buffer.name().empty()) {
		std::vector<char> all_names;
		buffer.get_names(all_names);
	...
	if (!buffer.only_continous()) {
		...
		if (nReturn)errormsg("XPRSgetintattrib", __LINE__, nReturn);
...
void XpressSolver::initLp(std::string const & name) {
	...
	if (nReturn)errormsg("XPRScreateprob", __LINE__, nReturn);
	...
	if (_stream.empty()) {
		...
		nReturn = XPRSsetintcontrol(_lp, XPRS_OUTPUTLOG, XPRS_OUTPUTLOG_FULL_OUTPUT);
		if (nReturn)errormsg("XPRSsetintcontrol", __LINE__, nReturn);
		nReturn = XPRSsetcbmessage(_lp, optimizermsg, this);
		if (nReturn)errormsg("XPRSsetcbmessage", __LINE__, nReturn);
	}

	nReturn = XPRSloadlp(_lp, name.c_str(), 0, 0, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL);
	if (nReturn)errormsg("XPRSloadlp", __LINE__, nReturn);
